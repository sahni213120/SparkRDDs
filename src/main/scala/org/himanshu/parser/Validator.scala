...
object Validator {
  ...
  def performValidations(row: String, delim: String, objectSchema: ObjectSchema): (String, String) = {

    val rowArray = row.split(delim);

    var errorCode = "";

    val schema = objectSchema.getSchema;

    errorCode = ValidationFunctions.isRowSizeValid(schema.size(), rowArray.size)
